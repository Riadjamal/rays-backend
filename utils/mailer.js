...
const transporter = nodemailer.createTransport({
    ...
    secure: process.env.EMAIL_PORT === '465' || !process.env.EMAIL_PORT, // Default to true if port is 465 or missing
    auth: {
...
exports.sendBookingConfirmation = async (email, bookingDetails) => {
    try {
        const mailOptions = {
            from: `"Rays International" <${process.env.EMAIL_USER}>`,
            to: email,
...
exports.sendVisaApproved = async (email, bookingDetails) => {
    try {
        ...
        console.error('Visa email failed:', error);
    }
};
...
exports.sendMail = async (options) => {
    try {
        const mailOptions = {
            from: `"Rays International Support" <${process.env.EMAIL_USER}>`,
            ...options
        };
